...
class ScaleProImageView @JvmOverloads constructor(
    ...
    private var currentScale = 0f
        set(value) {
            field = value.coerceIn(smallScale, bigScale)
            invalidate()
        }

    private val scaleFractionAnimator = ValueAnimator.ofFloat(0f, 0f).apply {
        duration = 200
        addUpdateListener {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        scaleFractionAnimator.setFloatValues(smallScale, bigScale)
    }
    ...
    private val scaleGesture = ScaleGestureDetector(context, object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
        override fun onScaleBegin(detector: ScaleGestureDetector): Boolean {
            touchOffsetX = (detector.focusX - width / 2f) * (1 - bigScale / smallScale)
            touchOffsetY = (detector.focusY - height / 2f) * (1 - bigScale / smallScale)
            return true
        }
            override fun onScale(detector: ScaleGestureDetector): Boolean {
                val temp = currentScale * detector.scaleFactor
                if(temp < smallScale || temp > bigScale){
                    return false
                }
            ...
            override fun onFling(
                ...
                if (big) {
                    ...
                    scroller.fling(
                        ...
                        velocityX.toInt(),
                        velocityY.toInt(),
                        -maxTouchOffsetX.toInt(),
                        maxTouchOffsetX.toInt(),
                        -maxTouchOffsetY.toInt(),
                        maxTouchOffsetY.toInt(),
                        100.dpView,
                        100.dpView
                    )
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        if(scaleGesture.isInProgress.not()){
